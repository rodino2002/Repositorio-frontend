...
export default function EditUser({ onClose, isOpen, itemSelected }: props) {
    ...
    useEffect(() => {
        ...
        setFormData({
            ...
            departamentoId: itemSelected.departamentoId
                ? String(itemSelected.departamentoId)
                : "",
            especialidadesIds: itemSelected.especialidades?.[0]?.id
            ? String(itemSelected.especialidades[0].id)
            : "",
            bi_number: itemSelected.bi_number ?? "",
        })
    }, [itemSelected])
    ...
    const updateUser = async (e: React.FormEvent) => {
        ...
        if(!itemSelected?.id) return toast.error("Usuário não encontrado!");
        ...
        try {
            ...
            await api.put(`usuarios/${itemSelected?.id}`, body)

            queryClient.invalidateQueries({ queryKey: [`gestaoContas`] });
            ...
            setLoading(false)
            if (isAxiosError(error)) {
                const message = error?.response?.data?.message ?? `Erro ao salvar alterações!`
    ...
    console.log(formData, itemSelected)
            ...
            <Sheet onOpenChange={onClose} open={isOpen}>
                ...
                <SheetContent style={{ minWidth: '35%' }} className="pr-16 pl-16 pt-10 w-full flex-col overflow-y-auto scrollbar-none">
                    <div className="flex justify-between items-center w-full pb-14">
                        <SheetHeader className="text-[#143163] font-semibold text-lg p-0">Editar conta</SheetHeader>
                    ...
                    <div className="flex flex-col space-y-2">
                        <label className="text-[#143163] text-[14px] font-semibold">
                            Tipo de conta
                            <strong className="text-[#ED5656]">*</strong>
                        </label>
                    ...
                    <form onSubmit={updateUser} className="">
                        ...
                        <div className="flex flex-col space-y-2 mt-6">
                            ...
                            <div className="flex justify-between items-center ">
                                ...
                                <input
                                    ...
                                    className={`w-full h-9 p-2 ring-1 rounded-l-[6px] ring-[#D4D9EA] focus:ring-1 bg-zinc-100 cursor-no-drop
                                     focus:ring-[#FFC505] focus:outline-none text-[#143163] text-sm`} />
                                <button type="button" onClick={() => ValidateBINumber(formData.bi_number)} disabled={true}
                                    className={`rounded-r-lg p-2 cursor-no-drop bg-green-300 text-green-600"  w-20 h-10`}>
                        ...
                        <div className="flex flex-col space-y-2 mt-6">
                            ...
                            <input type="text" disabled={true}  value={formData.nome} onChange={(e) => setFormData({ ...formData, nome: e.target.value })}
                                className={`bg-zinc-100 cursor-no-drop h-9 p-2 ring-1 rounded-[6px] ring-[#D4D9EA] focus:ring-1 focus:ring-[#FFC505] focus:outline-none text-[#143163] text-sm`} />
                        </div>
                        ...
                        <div className="flex flex-col space-y-2 mt-6 w-full">
                            ...
                            >
                                ...
                                <SelectContent>
                                    {listEspecialities?.length > 0 &&
                                        listEspecialities.map((item: any) => (
                                            <SelectItem
                                                key={item.id}
