...
export default function EditDepartament({ onClose, isOpen, itemSelected }: props) {
    ...
    const editDepartament = async (e: any) => {
        ...
        try {
            ...
            await api.put(`departamentos/${itemSelected?.id}`, body)

            queryClient.invalidateQueries({ queryKey: [`departamentoLista`] });


            setLoading(false)
            toast.success(`Depatarmento editado com sucesso!`, {
            ...
            if (isAxiosError(error)) {
                const message = error?.response?.data?.message ?? "Erro ao editar departamento"
